...
- 每个 sheet 列：书名、作者、类型、百度云盘、封面图片、添加时间
- 输出到 `result/YYYYMMDD-{数量}.xlsx`

### Excel 输出格式

| 书名 | 作者 | 类型 | 百度云盘 | 封面图片 | 添加时间 |
|------|------|------|---------|---------|---------|
| 黎东方讲史 | 黎东方 | 历史 | https://pan.baidu.com/s/... | https://teaax7941.../xxx.jpg | 2026-04-30 |
...
1. 加载 books.json 数据库
2. 按 type 字段分组
3. 每个类型创建一个 worksheet，列：书名、作者、类型、百度云盘、封面图片、添加时间
4. addedAt 格式化为 `YYYY-MM-DD`
5. 写入 `result/YYYYMMDD-{books.length}.xlsx`

- [ ] 列宽设置：书名 30、作者 20、类型 8、百度云盘 50、封面图片 50、添加时间 12
- [ ] 统计输出：总条目数、各类型条目数
...
- **Excel 导出**：新增 `export-excel` 命令，按类型分 sheet 生成 Excel 文件
  - 每个 sheet 包含：书名、作者、类型、百度云盘、封面图片、添加时间
  - 输出到 result/YYYYMMDD-{数量}.xlsx
